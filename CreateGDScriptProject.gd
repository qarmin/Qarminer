...
var list_of_all_files = {"2D": [], "3D": [], "Node": [], "Other": [], "Control": [], "Resource": [], "RefCounted": [], "Object": []}
...
func create_scene_files() -> void:
	...
	for type in ["2D", "3D", "Node", "Control", "Resource", "RefCounted", "Object"]:
...
[node name=\"Object\" parent=\".\" instantiate=ExtResource( 7 )]
[node name=\"2D\" parent=\".\" instantiate=ExtResource( 6 )]
[node name=\"3D\" parent=\".\" instantiate=ExtResource( 5 )]
[node name=\"Control\" parent=\".\" instantiate=ExtResource( 4 )]
[node name=\"Node\" parent=\".\" instantiate=ExtResource( 3 )]
[node name=\"RefCounted\" parent=\".\" instantiate=ExtResource( 2 )]
[node name=\"Resource\" parent=\".\" instantiate=ExtResource( 1 )]"""
	)
...
func create_basic_structure() -> void:
	...
	assert(directory.make_dir_recursive(base_path + "RefCounted/") == OK)
...
func get_object_folder(name_of_class: String) -> String:
	...
	if ClassDB.is_parent_class(name_of_class, "Node3D"):  # TODO Fix in Godot 4.0
		...
		return "Resource"
	elif ClassDB.is_parent_class(name_of_class, "RefCounted"):
		return "RefCounted"
	else:
		return "Object"
...
func create_basic_files() -> void:
	...
	for class_data in classes_thing:
		...
		var object_name
		if (
			ClassDB.can_instantiate(class_data.name)
			&& (
		...
		### Process Function
		if can_be_instanced || is_static:  # Disallow to use it for e.g. non instantable CollisionObject3D
			...
			if allow_to_replace_old_with_new_objects:
				...
				if ClassDB.is_parent_class(class_data.name, "Node"):
					data_to_save += "\t\t" + object_name + ".queue_free()\n"
				if can_be_instanced && !(ClassDB.is_parent_class(class_data.name, "RefCounted")) && !(ClassDB.is_parent_class(class_data.name, "Node")):
		...
		if !(class_data.name in ["PhysicsDirectBodyState3D", "PhysicsDirectSpaceState3D", "PhysicsDirectBodyState2D", "PhysicsDirectSpaceState2D", "TreeItem", "Image"]):  # Some functions are static, but some needs to work on objects etc.., TODO Remove Image when it will be enough stable
		...
		if can_be_instanced && !ClassDB.is_parent_class(class_data.name, "Node") && !ClassDB.is_parent_class(class_data.name, "RefCounted"):
...
func get_special_node(name_of_class : String) -> Node:
	assert(ClassDB.can_instantiate(name_of_class))
