...
func create_basic_files() -> void:
	...
	for class_data in CSharpCreateProjectBase.classes:
		...
		if ClassDB.is_parent_class(class_data.name, "Spatial"):  # TODO Fix in Godot 4.0
			...
			file_name += "Reference" + "/" + file_name_real + ".cs"
			CSharpCreateProjectBase.list_of_all_files["Reference"].append(file_name)
		else:
...
func _ready() -> void:
	...
	if Directory.new().dir_exists(CSharpCreateProjectBase.base_path):
