...
var debug_in_runtime: bool = true  # Allow to print info in runtime about currenty executed function, it is very helpful, so I don't recommend to turn this unchecked
...
func collect_data() -> void:
	...
	for name_of_class in BasicData.base_classes:
		...
		for method_data in method_list:
			...
			class_data.function_names.append(HelpFunctions.normalize_function_names(method_data["name"]))
...
func create_scene_files() -> void:
	...
	for type in ["2D", "3D", "Node", "Control", "Resource", "RefCounted", "Object"]:
		...
		file = FileAccess.open(base_path + str(type) + ".tscn", FileAccess.WRITE)
	...
	file = FileAccess.open(base_path + "All.tscn", FileAccess.WRITE)
	file.store_string(
...
#				print(file_name)
				assert(directory.remove_at(file_name) == OK)
...
#	print(to_delete)
	...
	assert(directory.remove_at(to_delete) == OK)
...
func create_basic_structure() -> void:
	...
	var file: FileAccess = FileAccess.open(base_path + "project.godot", FileAccess.WRITE)
	file.store_string(
		"""
config_version=4
