...
func _ready() -> void:
	...
	for i in range(number_of_nodes):
		...
		var child: Node = ClassDB.instance(BasicData.base_classes[index])
		var name_to_set : String = "Special Node " + str(i)
		child.set_name(name_to_set)
		if debug_enabled:
			to_print = "\tvar variable"+str(number_of_variables) + " = ClassDB.instance(\""+BasicData.base_classes[index]+"\")\n"
...
func _process(delta: float) -> void:
	...
	for i in range(5):
		...
		if debug_enabled:
			...
			to_print += "\tvar variable"+str(number_of_variables)+"parent = var variable"+str(number_of_variables)+"choosen.get_parent()#"+parent_of_node.get_class()+" - " + parent_of_node.get_name() +"\n"
			to_print += "\tvariable"+str(number_of_variables)+"random(variable"+str(number_of_variables)+"choosen = find_node("+random_node_name+", true, false) #"+random_node.get_class()+"\n"
...
#					to_print = "\tvar variable"+ # TODO
				var node: Node = ClassDB.instance(node_class)
...
#	yield(get_tree(), "idle_frame")  # TODO Add this to generated code
