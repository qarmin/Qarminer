...
func _process(delta: float) -> void:
	...
	for i in range(2):
		...
		if debug_enabled:
			...
			to_print += "\tvar " + choosen_node_variable_name + ' = find_nodes("' + choosen_node_name + '",Node,  true, false) #' + choosen_node.get_class() + "\n"
			...
			to_print += "\tvar " + random_node_variable_name + ' = find_nodes("' + random_node_name + '", true, Node, false) #' + random_node.get_class() + "\n"
...
#						choosen_node.set_name(temp_variable_name)
		...
		if choosen_node.find_nodes(random_node.get_name(),"Node", true, false) != null:  # Cannot set as node parent one of its child
			if debug_enabled:
