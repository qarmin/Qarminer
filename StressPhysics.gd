...
func random_functions() -> void:
	...
	for child in get_children():
		...
		for method_data in list_of_methods:
			...
			if ClassDB.class_has_method("Object", method_data.get("name")) || ClassDB.class_has_method("Node", method_data.get("name")):
				continue

			if method_data.get("name") in BasicData.function_exceptions:
				continue
			...
			child.callv(method_data.get("name"), arguments)
